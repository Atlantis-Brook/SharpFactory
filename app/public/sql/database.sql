...
CREATE TABLE IF NOT EXISTS `undertake_project`(
 ...
 CREATE TABLE IF NOT EXISTS `new_information`(
  ...
  `whatWillGet` TEXT NOT NULL COMMENT '期望结果',
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`id`)
)ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='新成员申请数据表' AUTO_INCREMENT=1;
...
-- 导出 message_board 表
 ...
 CREATE TABLE IF NOT EXISTS `message_board`(
  ...
  `radio` VARCHAR(120) NOT NULL COMMENT '同意选框',
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`id`)
)ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='留言板数据表' AUTO_INCREMENT=1;
